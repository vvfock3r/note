...
_start:
    mov rax, 60    ; 系统调用号，表示exit
    mov rdi, 0     ; 退出码，0表示正常退出
    syscall        ; 调用系统调用

; 第一条指令: mov rax, 60
; 1、将立即数 60 存储到 CPU 寄存器 rax 中, 这里的60是十进制数字60
; 2、立即数的意思是该数据直接嵌入到指令中，不需要从内存中读取
; 3、汇编指令的意义取决于它所处的上下文环境
; 4、在64位Linux中，exit系统调用的调用号是 60, 它对应我们的mov eax, 60中的立即数60

; 第二条指令: mov rdi, 0
; 1、将立即数 0 存储到 CPU 寄存器 rdi 中, 这里的0是十进制数字0
; 2、具体来说是设置退出状态码为0

; 第三条指令: syscall
; 发起一个Linux系统调用,其系统调用号存储在寄存器rax中。
; 在Windows操作系统中，使用int 0x80或int 0x2e指令来触发系统调用
...
; 3、搜索一下这个文件
;    [root@node-1 nasm]# find / -type f -name "unistd_64.h" | xargs grep "__NR_exit" --color=auto
;    #define __NR_exit 60
;    #define __NR_exit_group 231
; 4、查看一下文件
;    [root@node-1 nasm]# head /usr/include/asm/unistd_64.h
...
; 2、在这个头文件中，每个系统调用都有一个以 __NR_ 开头的宏定义，后面跟着系统调用的名称，比如 #define __NR_exit 1
; 3、搜索一下这个文件
;    [root@node-1 nasm]# find / -type f -name "unistd_32.h" | xargs grep "__NR_exit" --color=auto
;    #define __NR_exit 1
