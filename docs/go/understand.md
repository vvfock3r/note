...
```bash
# 数据寄存器, 其低16位分别称为 AX、BX、CX 和 DX
EAX - 扩展累加器是算术运算期间使用的主要寄存器。可用于存储函数调用的返回值或系统调用的功能编号            
EBX - 扩展基址寄存器用于存储数据地址。可用于存储系统调用参数
ECX - 扩展计数器寄存器用于在迭代循环期间存储计数
EDX - 扩展数据寄存器用于在输入/输出操作期间存储数据

# 索引寄存器, 其低16位称为 SI 和 DI
ESI - 扩展源索引寄存器用作流操作中指向源的指针
EDI - 扩展目标索引寄存器用作流操作中指向目标的指针

# 指针寄存器，其低16位称为 SP 和 BP
ESP - 扩展栈指针寄存器指向栈顶
EBP - 扩展基址指针寄存器指向栈底
```
